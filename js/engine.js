...
var Sprites = new function() {
  ...
  this.load = function(sprite_data,callback) { 
    ...
    this.image.src = 'images/sprites.png'; //the sprites image source 
  };

  this.draw = function(canvas,sprite,x,y,frame) {
    var s = this.map[sprite];
    if(!frame) frame = 0;
    canvas.drawImage(this.image, s.sx + frame * s.w, s.sy, s.w, s.h, x,y, s.w, s.h); //how the sprites are drawn on
  };
}
...
var GameScreen = function GameScreen(text,text2,callback) {
  ...
  this.render = function(canvas) {                        // formatting canvas
...
var GameBoard = function GameBoard(level_number) {
  ...
  this.collision = function(o1,o2) {        // the sprites moving left to right and not going off the canvas
  ...
  this.loadLevel = function(level) {   // this loads up the level and positions the player and aliens
    this.objects = [];
